  ...
  <div class="container">
      ...
      <form action="{{ route('admin.comics.update', $comic->id) }}" method="post" enctype="multipart/form-data">
          ...
          <div class="form-group">
              ...
              <textarea class="form-control" name="body" id="body" rows="5">{{$comic->body}}</textarea>
          </div>
          ...
          <select name="category" id="category">
              @foreach ($categories as $category)
                  <option {{ $comic->category_id == $category->id ? 'selected' : '' }} value="{{$category->id}}">{{$category->name}}</option>
              @endforeach
          </select>
